...
def test_metrics_written_after_fetch_batch(temp_metrics_file):
    ...
    # Call the metrics method directly
    client._record_freshness_metric(now, checked=5, fresh_count=3, cached_count=1, valid_count=4)
    
    # Verify the file exists and contains the expected record
...
def test_fresh_pct_calculation():
    ...
    for checked, fresh, expected_pct in test_cases:
        with patch.dict(os.environ, {"WINDOWBOT_METRICS_PATH": "/dev/null"}):
            client._record_freshness_metric(now, checked=checked, fresh_count=fresh, cached_count=0, valid_count=fresh)
        
        # Read last line to verify calculation
        # For /dev/null we can't read back, so let's verify the calculation in-memory
        if checked:
...
def test_metrics_appended_not_overwritten(temp_metrics_file):
    ...
    client._record_freshness_metric(now1, checked=5, fresh_count=1, cached_count=0, valid_count=1)
    client._record_freshness_metric(now2, checked=5, fresh_count=4, cached_count=0, valid_count=4)
